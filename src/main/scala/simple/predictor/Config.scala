...
object Config {
  ...
  val modelEpoch: Int = env("MODEL_EPOCH") flatMap(p => Try(p.toInt).toOption) getOrElse 0

  val modelEdge: Int = env("MODEL_EDGE") flatMap(p => Try(p.toInt).toOption) getOrElse 512
